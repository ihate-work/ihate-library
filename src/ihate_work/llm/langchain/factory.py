...
def get_embedding_model(model_spec: str, cache_path: str | Path | None = None, **kwargs) -> "Embeddings":
    ...
    if provider == "ollama":
        ...
        return _maybe_cached_embedding(
            OllamaEmbeddings(base_url=base_url or _get_ollama_base_url(), model=model, **kwargs),
...
def _get_ollama_base_url() -> str:
    ...
    if not base_url:
        raise ValueError(
            "OLLAMA_BASE_URL environment variable is not set. Please set it to the base URL of your Ollama API."
        )
    return base_url


def _maybe_cached_embedding(inner: Embeddings, *, model: str, cache_path: str | Path | None = None) -> Embeddings:
